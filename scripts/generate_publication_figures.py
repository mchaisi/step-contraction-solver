...
def example_output(key: str, filename: str) -> Path:
    archived = reference(key, filename)
    if archived.exists():
        return archived
    generated = RUN_OUTPUTS / RUNS[key] / filename
    if generated.exists():
        return generated
    raise FileNotFoundError(f"{archived} not found and no generated fallback at {generated}.")


def residual_history(key: str) -> tuple[np.ndarray, np.ndarray]:
    path = example_output(key, "run.log")
    steps: list[int] = []
...
def read_field(key: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    data = np.loadtxt(example_output(key, "step2-psi.dat"))
    x = np.unique(data[:, 0])
    y = np.unique(data[:, 1])
...
def main() -> int:
    ...
    for key in RUNS:
        summary = parse_log(example_output(key, "run.log"))
